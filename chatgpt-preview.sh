...
# v0.21.4p  nov/2023  by mountaineerbr  GPL+3
...
Synopsis
	...
	${0##*/} -i [opt] [L|P][hd] [PROMPT]  #dall-e-3
...
{
	[[ $1 = *[Hh][Dd]* ]] && { 	OPTS_HD="hd"; set -- "${1//[Hh][Dd]}" ;}  #defaults=standard
	case "$1" in  #width x height
		1024*1792|10*17*|[LlXx][Xx][Ll]|[Pp]ortrait) 			OPTS=1024x1792;;  #portrait
		1792*|17*|[Xx][Ll]|[Pp][Xx][Ll]|[Ll]andscape) 	OPTS=1792x1024;;  #landscape
		1024*|10*|[Ll][Aa][Rr][Gg][Ee]|[Ll]| $([[ $MOD_IMAGE = *[3-9] ]] && echo '*') ) 	OPTS=1024x1024;;
...
do
	...
	case "$opt" in
		...
		e) 	OPTE=1 EPN=2;;
		E) 	OPTEXIT=1;;
		f$OPTF) unset EPN MOD MOD_CHAT MOD_EDIT MOD_AUDIO MOD_IMAGE MODMAX INSTRUCTION OPTC OPTE OPTI OPTLOG USRLOG OPTRESUME OPTCMPL MTURN OPTTIKTOKEN OPTTIK OPTYY OPTFF OPTK OPTHH OPTL OPTMARG OPTMM OPTNN OPTMAX OPTA OPTAA OPTB OPTBB OPTN OPTP OPTT OPTV OPTVV OPTW OPTWW OPTZ OPTSTOP OPTCLIP CATPR OPTCTRD OPT_AT_PC OPT_AT Q_TYPE A_TYPE RESTART START STOPS OPTSUFFIX SUFFIX CHATGPTRC CONFFILE REC_CMD STREAM OPTEXIT API_HOST GPTCHATKEY
...
then 	((OPTI+OPTII)) && MOD_IMAGE=$OPTMARG  #image
	...
	then 	MOD=$MOD_CHAT
	elif ((OPTW)) && ((!MTURN))  #audio endpoint
	then 	MOD=$MOD_AUDIO
	fi
fi
((${#MOD}<3)) && pick_modelf 
